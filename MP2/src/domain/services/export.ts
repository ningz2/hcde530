...
export async function buildPrintablePdfBuffer(workspaceId: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.font("Helvetica-Bold").fontSize(20).fillColor(PDF_COLORS.ink).text(view.workspaceName ?? "Affinity board");
    doc
      .font("Helvetica")
      .fontSize(10)
      .fillColor(PDF_COLORS.muted)
      .text(`${view.codeCount} codes · ${view.themes.length} groups · generated by AffinityFlow`, { lineGap: 4 });

    for (const theme of view.themes) {
      doc.moveDown(1.4);
      ensureSpace(doc, 80);
      doc.font("Helvetica-Bold").fontSize(14).fillColor(PDF_COLORS.ink).text(theme.title);
      doc.moveDown(0.35);
...
const NOTE_GAP = 9;
const NOTE_MIN_WIDTH = 165;
const NOTE_PADDING = 9;
const NOTE_TOP_ACCENT = 4.5;
...
function measureNoteCardHeight(
  ...
  let height = NOTE_PADDING * 2 + NOTE_TOP_ACCENT + 6;

  doc.font("Helvetica-Bold").fontSize(15);
  height += doc.heightOfString(assignment.code, { width: innerWidth }) + 6;

  doc.font("Helvetica").fontSize(12);
  for (const field of noteFields(assignment)) {
    height += doc.heightOfString(`${field.label} ${field.value}`, { width: innerWidth, lineGap: 2 }) + 6;
  }
...
function drawNoteCard(
  ...
  doc.font("Helvetica-Bold").fontSize(15).fillColor(PDF_COLORS.ink);
  ...
  for (const field of noteFields(assignment)) {
    ...
    doc.font("Helvetica").text(field.value, { width: innerWidth, lineGap: 2 });
    textY = doc.y + 6;
  }
}
