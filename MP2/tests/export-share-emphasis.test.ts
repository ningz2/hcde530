...
async function seedWorkspaceWithBoard(optOut = false): Promise<string> {
  ...
  await applyAnonymization({ workspaceId, applyMasking: !optOut });
...
describe("export scaffolds", () => {
  ...
  it("generates a synchronous CSV artifact", async () => {
    ...
    const job = await requestExport({ workspaceId, requestedByUserId: "u1", format: "CSV" });
  ...
  it("generates downloadable PDF and FigJam artifacts", async () => {
    const workspaceId = await seedWorkspaceWithBoard();
    const pdf = await requestExport({ workspaceId, requestedByUserId: "u1", format: "PDF" });
    expect(pdf.status).toBe("READY");
    expect(pdf.artifactFilename).toBe("affinity-board.pdf");
    expect(pdf.artifactMimeType).toBe("application/pdf");
    expect(Buffer.from(pdf.artifactPreview ?? "", "base64").subarray(0, 4).toString()).toBe("%PDF");

    const figjam = await requestExport({ workspaceId, requestedByUserId: "u1", format: "FIGJAM" });
    expect(figjam.status).toBe("READY");
    expect(figjam.artifactFilename).toBe("figjam-sticky-notes.csv");
    expect(figjam.artifactPreview).toContain("section,group,sticky_note,participant,color");
    expect(figjam.artifactPreview).toContain("Pricing unclear");
  });

  it("rejects export when no board exists", async () => {
    const workspaceId = (
      await repo.createWorkspace({
        name: "WS",
        defaultHierarchyDepth: 2,
    ...
    await expect(requestExport({ workspaceId, format: "CSV" })).rejects.toThrow(ApiError);
  });
...
describe("anonymous view-only redaction", () => {
  ...
  it("hides unmasked codes from anonymous viewers", async () => {
    ...
    const memberView = await getBoardView(workspaceId);
    const anonView = await getBoardView(workspaceId, { redactUnmasked: true });
...
describe("share link lookup", () => {
  ...
  it("matches an active link by token hash and rejects unknown tokens", async () => {
    const workspaceId = (
      await repo.createWorkspace({
        name: "WS",
        defaultHierarchyDepth: 2,
    ...
    expect(await repo.findActiveShareLink(workspaceId, tokenHash)).toBeDefined();
    expect(await repo.findActiveShareLink(workspaceId, "wrong")).toBeUndefined();
  });
});
